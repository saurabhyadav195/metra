/**
 * METRA — components/evaluations/forms/DiscriminationTestForm.tsx
 * Discrimination Test (OIML R 76-1 §A.4.8.2)
 *
 * Captures the sensitivity / discrimination observations:
 *   - Pre-load indication (I_before) at test load
 *   - Indication after adding extra weight 1.4d (I_after)
 *   - For each test load point (typically 3 loads: ~5% Max, ~50% Max, ~Max)
 *
 * Backend checks: I_after >= I_before + d
 */
...
export interface DiscriminationTestFormProps {
  testId: string;
  testName: string;
...
const DEFAULT_ROWS: DiscriminationRow[] = [
  { load_label: "~5% Max", test_load: 0, I_before: 0, I_after: 0 },
];

export function DiscriminationTestForm({
  observations,
  onObservationsChange,
  disabled = false,
}: DiscriminationTestFormProps) {
  const [d, setD] = useState<number>(observations?.d !== undefined ? Number(observations.d) : 0.01);

  const parseRows = (): DiscriminationRow[] => {
    if (observations?.rows && Array.isArray(observations.rows) && observations.rows.length > 0) {
      return observations.rows.map((r: any) => ({
        load_label: String(r.load_label ?? ""),
  ...
  const [rows, setRows] = useState<DiscriminationRow[]>(parseRows);

  useEffect(() => {
    onObservationsChange({ d, rows });
  }, [d, rows]);

  const handleChange = (idx: number, field: keyof DiscriminationRow, val: string) => {
    const updated = [...rows];
    if (field === "load_label") {
      updated[idx] = { ...updated[idx], load_label: val };
    } else {
  ...
  const handleAddRow = () => {
    setRows([...rows, { load_label: "", test_load: 0, I_before: 0, I_after: 0 }]);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
        <div>
          <h4 className="text-xs font-semibold text-foreground">Discrimination Test — Before/After Observations</h4>
          <p className="text-[11px] text-muted-foreground">
            OIML R 76-1 §A.4.8.2 — Add 1.4d extra load; verify indication changes by at least d
          </p>
        </div>
      ...
      <div className="rounded-md border border-primary/30 bg-primary/5 p-3 space-y-1 max-w-xs">
        ...
        <p className="text-[10px] text-muted-foreground">Extra load = 1.4 × d; expected change ≥ d</p>
        ...
        <p className="text-[11px] font-mono text-primary">Extra load = {(1.4 * d).toFixed(4)} kg</p>
      </div>
        ...
        <table className="w-full text-left text-xs">
          <thead>
            <tr className="border-b border-border bg-muted/40 font-medium text-muted-foreground">
              <th className="py-2.5 px-3">Load Label</th>
              <th className="py-2.5 px-3">Test Load (L) [kg]</th>
              <th className="py-2.5 px-3">Indication Before (I₁) [kg]</th>
              <th className="py-2.5 px-3">Indication After +1.4d (I₂) [kg]</th>
            ...
            {rows.map((row, idx) => (
              <tr key={idx} className="hover:bg-muted/20 transition-colors">
                <td className="py-1.5 px-3">
                  <Input type="text" value={row.load_label}
                    onChange={(e) => handleChange(idx, "load_label", e.target.value)}
                    disabled={disabled} className="h-7 w-24 text-xs" placeholder="e.g. ~50% Max" />
                </td>
                <td className="py-1.5 px-3">
